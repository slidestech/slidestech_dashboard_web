...
class NotificationController extends Controller
{
        ...
        if ($role == 'user') {
            ...
            $agents = Agent::with('level', 'department', 'fonction', 'confirmedLevels')
                ->whereHas('confirmedLevels', function ($query) {
                    $query->whereRaw("state = 'VALIDE'");
                })->whereHas('department', function ($query) use ($userStructureId, $current_date) {
    ...
    public function sendNotification()
    {
        ...
        // Broadcast the event to the connected clients
        // broadcast($event);
        event($event);

        // Return a response indicating that the message was sent
