...
@section('page_content')
                        ...
                        <form>
                            <div class="row" id="add-technicalControl-form">
                                <div class="form-group col-md-12 m-t-15">
                                    <label for="-date" class="block">Véhicule <span class="text-danger">
                                            (*)</span></label>
                                    <div :class="[errors.vehicle_id ? '  input-group input-group-danger' :
                                        ...
                                        <select id="vehicles" class="selectpicker show-tick text-center"
                                            title="Sélectionner un véhicule.." data-width="100%">
                                ...
                                <div class="form-group col-md-7 m-t-15">
                                    <label for="startedat" class="block">Date du controle<span class="text-danger">
                                            (*)</span></label>
                                    <div
                                        :class="[errors.performed_at ? ' input-group input-group-danger' :
                                            ' input-group input-group-inverse'
                                        ]">
                                        <input id="performed_at" type="text" class="form-control date text-center"
                                            placeholder="Date du controle..." data-toggle="tooltip" data-placement="top"
                                            :data-original-title="errors.performed_at">
                                        <span class="input-group-addon">
                                ...
                                <div class="form-group col-md-5 m-t-15">
                                    ...
                                    <div
                                        :class="[errors.cost ? '  input-group input-group-danger' :
                                            ' input-group input-group-inverse'
                                        ]">
                                        <input type="number" class="form-control " placeholder="Coût..."
                                            data-toggle="tooltip" data-placement="top" :data-original-title="errors.cost"
                                            v-model="cost" min="0">
                                        <span class="input-group-addon">
                                ...
                                <div class="form-group col-md-7 m-t-15">
                                    <label for="ends_at" class="block">Date du prochaine révision<span
                                            class="text-danger"> (*)</span></label>
                                    <div
                                        :class="[errors.ends_at ? ' input-group input-group-danger' :
                                            ' input-group input-group-inverse'
                                        ]">
                                        <input id="ends_at" type="text" class="form-control date text-center"
                                            placeholder="Date du réparationdu prochaine révision..." data-toggle="tooltip"
                                            data-placement="top" :data-original-title="errors.ends_at">
                                        <span class="input-group-addon">
                                            <strong class="icofont icofont-social-slack"></strong>
                                        </span>
                                    </div>
                                ...
                                <div class="form-group col-md-12 m-t-15">
                                    <label for="details" class="block">SOUS-RESERVES <span class="text-danger">(*)
                                        </span></label>
                                    <input type="checkbox" class="js-small js-inverse" v-model="has_details"
                                        data-switchery="true">
                                    <div
                                        :class="[errors.details ? '  input-group input-group-danger' :
                                            ' input-group input-group-inverse'
                                        ]">
                                        <textarea rows="5" cols="5" class="form-control" placeholder="Les résevres..." placeholder="Détails..."
                                            data-toggle="tooltip" data-placement="top" :data-original-title="errors.details" v-model="details"
                                            :disabled="!has_details"></textarea>
                                    </div>
                                </div>
                        ...
                        <ul class="list-unstyled card-option">
                            <li>
                                <span data-toggle="tooltip" data-placement="top"
                                    data-original-title="Ajouter un controle technique">
                                    <i class="fa fa-plus faa-horizontal animated text-success " style="font-size:22px"
                                        data-toggle="modal" data-target="#technicalControl-modal"
    ...
    technicalControl.vehicle.model.name.toUpperCase() + ' - ' + technicalControl.vehicle.licence_plate }}</td>
                                    ...
                                    <td v-if="!technicalControl.has_details" class="text-center"><i
                                            class="icofont icofont-ui-check text-success"></i>
                                        <p style="display:none">Oui</p>
                                    </td>
                                    <td v-else class="text-center"><i class="icofont icofont-ui-close text-danger"></i>
                                        <p style="display:none">Non</p>
                                    </td>

                                    <td v-if="technicalControl.details" class="text-left"
                                        style="width:200px;white-space:normal;">@{{ technicalControl.details.toUpperCase() }}</td>
                                    <td v-else class="text-left" style="width:100px;white-space:normal;"></td>

                                    <td style="width:70px">
                                        <div class="text-center">
                                            <span data-toggle="modal" data-target="#technicalControl-modal"
                                                v-on:click="edit_technicalControl(technicalControl,index)">
                                            ...
                                            <i class="feather icon-trash text-danger f-18 clickable"
                                                v-on:click="delete_technicalControl(technicalControl.id,index)"
                                                data-toggle="tooltip" data-placement="top"
        ...
        const app = new Vue({
            ...
            methods: {
                    ...
                    $('#' + tableName).DataTable({
                        "footerCallback": function(row, data, start, end, display) {
                            ...
                            $(api.column(3).footer()).html(
                                ...
                                ' (TOTAL GLOBAL : ' + formatMoney(total) + ' DA )'
                            );
                        },
                        dom: 'Bfrtip',
                        language: {
                                    ...
                                    .then(function(response) {
                                        if (response.data.error) {
                                            ...
                                            notify('Succès', response.data.success, 'green', 'topCenter',
                                                'bounceInDown');
                                            app.fill_table('/getTechnicalControls',
                                                'technicalControls-table');
                                            app.reset_form();
                                        }
                                    });
                            }
                ...
                edit_technicalControl(technicalControl, index) {
                    ...
                    $('#performed_at').data('daterangepicker').setStartDate(reFormatDate(technicalControl
                        .performed_at));
                    $('#performed_at').data('daterangepicker').setEndDate(reFormatDate(technicalControl
                        .performed_at));
